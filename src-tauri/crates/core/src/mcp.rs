...
pub fn notify_tool() -> Value {
    json!({
        "name": "notify",
        "description": "Leaves a few words on this session's row in the user's sessions list, and marks the row as wanting attention unless the user is already looking at it. Call it as the last step of every turn that ends with you handing back to the user: what is done, why you stopped, or what you need from them. You cannot tell whether the user is watching, so call it regardless. The row has room for about forty characters on one line and cuts the rest off, so write it like a commit subject: 'Tests green, ready to merge', 'Need the API key'. Once per turn, at the end. Not for progress, and not while you are still working.",
        "inputSchema": {
            "type": "object",
            "properties": {
